...
def leer_nivel():
    ...
    for _ in range(5):
        ...
        time.sleep(0.06)
    if not lecturas:
        if _nivel_previo >= 0 and _rechazos_consecutivos < NIVEL_MAX_RECHAZOS * 3:
            _rechazos_consecutivos += 1
            print(f"[nivel] sin ecos válidos ({_rechazos_consecutivos}/{NIVEL_MAX_RECHAZOS*3}), usando previo={_nivel_previo} m")
